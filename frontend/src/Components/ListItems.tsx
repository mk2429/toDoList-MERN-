...
export const ListItems = (props: allTaskArrayType) => {
    ...
    function makeListItem({ item, index }: makeListItemType) {
        ...
        return (
            <div className="lidiv">
                <div className="liItem" style={{ background: "#d9dff8" ,display:"flex",alignItems:'center'}} id={itemDivId} onMouseEnter={() => mouseEnterLiDiv(itemDivId)} onMouseLeave={() => mouseLeaveLiDiv(itemDivId)}>
                    <div key={index} id={itemId} style={{ color: "black", width: "80%", wordWrap: "break-word" }} onClick={() => toggleLineThrough(itemId)}>
                        {item}
                        </div> 
